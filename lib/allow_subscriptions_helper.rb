			...
			def link_to_subscription(object, options={})
				...
				if logged_in?
					if object.subscribed_by?(current_user)
						link_to_unsubscribe(object, options.merge!(:unsubscribe_text => options[:unsubscribe_text]))
					else
						link_to_subscribe(object, options.merge!(:subscribe_text => options[:subscribe_text]))
					end
				else
					options[:logged_out_text]
				end
